<template>
  <div>
      ...
      <el-table :data="tableData" border stripe>
        ...
        >
          <template slot-scope="scope">
            <el-tag v-if="scope.row.type === 'master'" type="success"
              >master</el-tag
            >
        ...
        <el-table-column label="操作" align="center" header-align="center">
          <template slot-scope="scope">
            ...
            <el-button
              ...
              size="mini"
              @click="
                dialogTableVisible_instance_monitor = true;
                rowid = scope.row.id;
              "
              >实例监控
            </el-button>
    ...
    >
      ...
      >
        ...
        <el-form-item label="数据库类型" prop="db_type">
          ...
          <el-radio v-model="formData.db_type" label="distributed_mysql"
            >distributed_mysql</el-radio
          >
          <el-radio v-model="formData.db_type" label="cassandra"
            >cassandra</el-radio
          >
          <el-radio v-model="formData.db_type" label="pgsql">pgsql</el-radio>
          <el-radio v-model="formData.db_type" label="mongodb"
            >mongodb</el-radio
          >
        ...
        <el-form-item>
          <el-row :gutter="15">
            <el-col :span="10">
              ...
              <el-button v-if="autofocus" @click="resetForm" type="info"
                >重置</el-button
              >
            </el-col>
            <el-col :span="13">
              <el-button @click="headlerCheck('formRef')" type="info"
                >连接测试</el-button
              >
              <el-button type="primary" @click="handlerCommit('formRef')"
                >确 定</el-button
              >
            </el-col>
          </el-row>
    ...
    <el-dialog
      title="实例监控"
      :visible.sync="dialogTableVisible_instance_monitor"
    >
      <el-table :data="instance_monitor_data">
        ...
        <el-table-column label=" -innodb rows status- ">
          <el-table-column
            property="innodb_rows_inserted_diff"
            label="ins"
          ></el-table-column>
          <el-table-column
            property="innodb_rows_updated_diff"
            label="upd"
          ></el-table-column>
          <el-table-column
            property="innodb_rows_deleted_diff"
            label="del"
          ></el-table-column>
          <el-table-column
            property="innodb_rows_read_diff"
            label="read"
          ></el-table-column>
        </el-table-column>
        ...
        <el-table-column label="--innodb log--">
          <el-table-column
            property="innodb_os_log_fsyncs_diff"
            label="fsyncs"
          ></el-table-column>
          <el-table-column
            property="innodb_os_log_written_diff"
            label="written"
          ></el-table-column>
        </el-table-column>
...
export default {
  data() {
    var checkHost = (rule, value, callback) => {
      ...
      var rule =
        /(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)|((?:[a-zA-Z0-9](?:[a-zA-Z0-9\-]{0,61}[a-zA-Z0-9])?\.)+[a-zA-Z0-9][a-zA-Z0-9-]{0,61}[a-zA-Z0-9])/g;
      // var rule = /\d+\.\d+\.\d+\.\d+/g;
      if (!rule.exec(value)) {
    ...
    return {
      ...
      formRule: {
        ...
        type: [
          { required: true, message: "请选择主从类型", trigger: "change" },
        ],
        db_type: [
  ...
  methods: {
    ...
    handlerCommit(form) {
      this.$refs[form].validate(async (valid) => {
        ...
        if (this.autofocus) {
          var { data: res } = await this.$ajax
            .post("/instance/", this.formData)
            .catch(() => {
    ...
    async del_binlog() {
      if (!this.formData.binlog)
        return this.$message.error("请选择要清理的binlog");
      const { data: tab } = await this.$ajax.post("/del_binlog/", {
        binlog: this.formData.binlog,
        id: this.formData.id,
      });
    ...
    async instance_monitor() {
      if (this.dialogTableVisible_instance_monitor) {
        const { data: tab } = await this.$ajax.get(
          `/instance_monitor/?id=${this.rowid}`
        );
